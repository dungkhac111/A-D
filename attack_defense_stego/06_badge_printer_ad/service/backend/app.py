...
CHECKER_TOKEN=os.environ.get("CHECKER_TOKEN","checker-secret"); CHALLENGE="badge"; NAME="Badge Printer AD"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/": return self.send_text(render("index.html",name=html.escape(NAME),technique="JWT alg=none + IDOR + QR",probe="/login"),"text/html; charset=utf-8")
...
if __name__=="__main__": ensure_state(); port=int(os.environ.get("PORT","8089")); print(f"Badge Printer AD listening on 0.0.0.0:{port}"); ThreadingHTTPServer(("0.0.0.0",port),Handler).serve_forever()
