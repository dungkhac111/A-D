...
CHECKER_TOKEN=os.environ.get("CHECKER_TOKEN","checker-secret"); CHALLENGE="qr"; NAME="QR Shadow Layer AD"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/": return self.send_text(render("index.html",name=html.escape(NAME),technique="QR SVG shadow layer",probe="/badge?id=latest"),"text/html; charset=utf-8")
...
if __name__=="__main__": ensure_state(); port=int(os.environ.get("PORT","8086")); print(f"QR Shadow Layer AD listening on 0.0.0.0:{port}"); ThreadingHTTPServer(("0.0.0.0",port),Handler).serve_forever()
