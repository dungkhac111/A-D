...
SERVICE_PORT=int(os.environ.get("SERVICE_PORT","8087")); CHECKER_TOKEN=os.environ.get("CHECKER_TOKEN","checker-secret"); TIMEOUT=int(os.environ.get("CHECKER_TIMEOUT","15")); FLAG_PREFIX=os.environ.get("FLAG_PREFIX","blockChainPTIT{"); ACTIONS={"check","put","get"}
...
def check(base):
    ...
    if health.get("ok") is not True or health.get("challenge")!="css": raise Mumble("/healthz returned unexpected challenge data")
    status,_h,body=request(base,"/",token=False)
    if status!=200: raise Mumble(f"homepage returned HTTP {status}")
    if "CSS Color Encoding AD" not in body.decode("utf-8",errors="replace"): raise Mumble("homepage marker is missing")
    status,_h,_b=request(base,"/debug/theme?name=internal",token=False)
